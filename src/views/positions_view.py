...
def generate_positions_view(positions: list[Position], fx_rate_provider, output_path: str):
    ...
    for pos in positions:
        ...
        if pos.closed:
            ...
            data.append(["CLOSED", pos.symbol, pos.currency, pos.quantity, nm(exposure),
                         ...
                         nm(dividends), nm(taxes), nm(profit), duration_days, nm(annualized)])
        else:
            data.append(["", pos.symbol, pos.currency, "", pos.quantity, pos.open_date.date(), pos.buy_price, nm(cost), "", "", "", 
    ...
    data.insert(0, 
        ["Closed", "Ticker", "Currency", "Quantity", f"Capital Exposure ({fx_rate_provider.base_currency} * years)", 
         ...
         f"Net Profit/Loss ({fx_rate_provider.base_currency})", "Holding period (days)", "Annual Return"])
